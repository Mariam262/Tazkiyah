...
const StudentTraining = ({fetchMentorTraining, setFetchMentorTraining}) => {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_BACKEND_PORT}/upload/tarbiyah`, {
            headers: {
    ...
    const markRead = (id) => {
        axios.patch(`${process.env.REACT_APP_BACKEND_PORT}/upload/${id}/${userData.userId}/${userData.isMentor ? 'tarbiyah' : ''}`, {
            headers: {
                'Content-Type': 'application/json'
            }
        }).then(() => {
    ...
    return (
        <div>
            <div style={{ display: "flex", justifyContent: "center" }}>
                <h1 className="text-3xl pt-6 pb-5">Mentor Training Material</h1>
            </div>
